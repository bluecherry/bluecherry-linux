...
This library is used to communicate as a BlueCherry Lite device to the platform. The underlying
protocol is CoAP + DTLS but on the application level it looks as if you are talking MQTT.
This makes programming your application very straigtforward while making it very low bandwith
and compatible with high latency links.
...
certificate you can connect your device to the platform after you have a network connection. Connecting
to the platform is now done in only a few lines of code:

```C
void msg_handler(uint8_t topic, uint16_t len, const uint8_t *data, void *args)
{
  printf("Received MQTT message of length %d on topic %02X: %.*s", len, topic, len, data);
}
